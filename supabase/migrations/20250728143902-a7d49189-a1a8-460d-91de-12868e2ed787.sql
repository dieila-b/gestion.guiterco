...
-- Grant all permissions to Administrateur role
...
-- Grant Manager permissions (everything except user management)
INSERT INTO public.role_permissions (role_id, permission_id, can_access)
SELECT r.id, p.id, true
FROM public.roles r
CROSS JOIN public.permissions p
...
OR (p.menu = 'Paramètres' AND p.submenu != 'Utilisateurs' AND p.submenu != 'Rôles et permissions')
ON CONFLICT (role_id, permission_id) DO UPDATE SET can_access = true;

-- Grant Vendeur permissions (sales, clients, catalogue)
INSERT INTO public.role_permissions (role_id, permission_id, can_access)
SELECT r.id, p.id, true
FROM public.roles r
CROSS JOIN public.permissions p
WHERE r.name = 'Vendeur'
AND p.menu IN ('Tableau de bord', 'Ventes', 'Clients', 'Catalogue')
AND p.action IN ('read', 'write')
ON CONFLICT (role_id, permission_id) DO UPDATE SET can_access = true;

-- Grant Caissier permissions (caisse, clients read only)
INSERT INTO public.role_permissions (role_id, permission_id, can_access)
SELECT r.id, p.id, true
FROM public.roles r
CROSS JOIN public.permissions p
...
AND (
  p.menu = 'Caisse'
  OR (p.menu = 'Tableau de bord' AND p.action = 'read')
  OR (p.menu = 'Clients' AND p.action = 'read')
  OR (p.menu = 'Catalogue' AND p.action = 'read')
)
ON CONFLICT (role_id, permission_id) DO UPDATE SET can_access = true;