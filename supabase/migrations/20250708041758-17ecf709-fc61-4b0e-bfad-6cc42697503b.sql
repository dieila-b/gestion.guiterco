...
-- 1. Recréer la vue vue_factures_impayees_summary avec le bon calcul du nombre d'articles
...
SELECT 
    ...
    CASE 
        WHEN COALESCE(versements.total_paye, 0) = 0 THEN 'en_attente'
        WHEN COALESCE(versements.total_paye, 0) < fv.montant_ttc THEN 'partiellement_payee'
        ELSE 'paye'
...
WHERE fv.statut_paiement IN ('en_attente', 'partiellement_payee')
   OR (COALESCE(versements.total_paye, 0) < fv.montant_ttc AND fv.statut_paiement != 'paye')
ORDER BY fv.date_facture DESC;

-- 2. Créer une fonction pour récupérer les factures impayées avec tous les détails
...
WHERE fv.statut_paiement IN ('en_attente', 'partiellement_payee')
   OR (COALESCE(versements_data.total_paye, 0) < fv.montant_ttc AND fv.statut_paiement != 'paye')
ORDER BY fv.date_facture DESC;
$$;
