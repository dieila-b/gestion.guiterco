...
-- Créer des politiques pour sorties_stock si elle existe
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'sorties_stock' AND table_schema = 'public') THEN
        DROP POLICY IF EXISTS "Authenticated users can read sorties_stock" ON public.sorties_stock;
        DROP POLICY IF EXISTS "Authenticated users can write sorties_stock" ON public.sorties_stock;
...
-- Créer des politiques pour transferts si elle existe
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transferts' AND table_schema = 'public') THEN
        DROP POLICY IF EXISTS "Authenticated users can read transferts" ON public.transferts;
        DROP POLICY IF EXISTS "Authenticated users can write transferts" ON public.transferts;
