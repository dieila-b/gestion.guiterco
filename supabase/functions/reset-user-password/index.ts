...
serve(async (req) => {
  ...
  try {
    ...
    // Récupérer l'utilisateur interne spécifique
    ...
    if (fetchError || !internalUser) {
      ...
      throw new Error(`Utilisateur non trouvé ou inactif: ${fetchError?.message}`);
    }
    ...
    // Générer un mot de passe temporaire visible
