...
export const useCreateFactureVente = () => {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      ...
      // Determine statut_livraison with proper typing
      const isLivree = data.payment_data?.statut_livraison === 'livre' || 
                      data.payment_data?.statut_livraison === 'livree';
      
      const statutLivraison: 'En attente' | 'Partiellement livrée' | 'Livrée' = 
        isLivree ? 'Livrée' : 'En attente';
      ...
      const factureData = {
        ...
        statut_livraison_id: isLivree ? 3 : 1,
      ...
      const lignesFacture = data.cart.map((item: any) => ({
        ...
        quantite_livree: isLivree ? item.quantite : 0,
        statut_livraison: isLivree ? 'Livrée' : 'En attente'
      }));
